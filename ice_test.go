...
var iceAuthTestCases = []iceAuthTestCase{
	...
	},
	{
		...
		config: []*stnrv1.StunnerConfig{&staticAuthConfig},
		params: "service=turn&publicIp=1.3.5.7",
		status: 200,
		tester: func(t *testing.T, iceConfig *types.IceConfig, authHandler a12n.AuthHandler) {
			assert.NotNil(t, iceConfig, "ICE config nil")
			assert.NotNil(t, iceConfig.IceServers, "ICE servers nil")
	...
	{
		name:   "static - public port set via URL parameter",
		config: []*stnrv1.StunnerConfig{&staticAuthConfig},
		params: "service=turn&publicPort=1234",
		status: 200,
		tester: func(t *testing.T, iceConfig *types.IceConfig, authHandler a12n.AuthHandler) {
			assert.NotNil(t, iceConfig, "ICE config nil")
			assert.NotNil(t, iceConfig.IceServers, "ICE servers nil")
			...
			uris := *iceAuth.Urls
			assert.Len(t, uris, 4, "URI len")
			assert.Contains(t, uris, "turn:1.2.3.4:1234?transport=udp", "UDP URI")
			assert.Contains(t, uris, "turn:1.2.3.4:1234?transport=tcp", "TCP URI")
			assert.Contains(t, uris, "turns:127.0.0.1:1234?transport=tcp", "TLS URI")
			assert.Contains(t, uris, "turns:127.0.0.1:1234?transport=udp", "DTLS URI")
		},
	},
	{
		name:   "static - public IP and port set via URL parameters",
		config: []*stnrv1.StunnerConfig{&staticAuthConfig},
		params: "service=turn&publicIp=1.3.5.7&publicPort=1234",
		status: 200,
		tester: func(t *testing.T, iceConfig *types.IceConfig, authHandler a12n.AuthHandler) {
			assert.NotNil(t, iceConfig, "ICE config nil")
			assert.NotNil(t, iceConfig.IceServers, "ICE servers nil")
			...
			uris := *iceAuth.Urls
			assert.Len(t, uris, 4, "URI len")
			assert.Contains(t, uris, "turn:1.3.5.7:1234?transport=udp", "UDP URI")
			assert.Contains(t, uris, "turn:1.3.5.7:1234?transport=tcp", "TCP URI")
			assert.Contains(t, uris, "turns:1.3.5.7:1234?transport=tcp", "TLS URI")
			assert.Contains(t, uris, "turns:1.3.5.7:1234?transport=udp", "DTLS URI")
		},
	},
}
